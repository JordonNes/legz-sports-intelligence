...
class LiveSportsClient:
    ...
    def _get_response(
        ...
        try:
            ...
            try:
                ...
                provider_code = provider_payload.get("error_code") or provider_payload.get("message")
            except ValueError:
                provider_code = None
    ...
    def schedules(self, league: str, on_date: date | None = None) -> dict[str, Any]:
        ...
        raw = self._get(
            f"https://site.api.espn.com/apis/site/v2/sports/{sport}/{espn_league}/scoreboard",
            params=params,
        )
        events = []
            ...
            events.append(
                {
                    ...
                    "broadcasts": [b.get("names", []) for b in competition.get("broadcasts", [])],
                    "odds": odds,
                }
            )
    ...
    def _mlb_schedule(self, on_date: date | None) -> dict[str, Any]:
        ...
        raw = self._get(
            ...
            params={"sportId": 1, "date": target, "hydrate": "probablePitcher,team,venue,linescore"},
        )
        events = []
        for bucket in raw.get("dates", []):
            for game in bucket.get("games", []):
                ...
                events.append(
                    {
                        "id": game.get("gamePk"),
                        "name": f"{teams.get('away', {}).get('team', {}).get('name')} at {teams.get('home', {}).get('team', {}).get('name')}",
                        "start_time": game.get("gameDate"),
                        "status": (game.get("status") or {}).get("detailedState"),
                        "home": {
                            "name": teams.get("home", {}).get("team", {}).get("name"),
                            "score": teams.get("home", {}).get("score"),
                            "probable_pitcher": (teams.get("home", {}).get("probablePitcher") or {}).get("fullName"),
                        },
                        "away": {
                            "name": teams.get("away", {}).get("team", {}).get("name"),
                            "score": teams.get("away", {}).get("score"),
                            "probable_pitcher": (teams.get("away", {}).get("probablePitcher") or {}).get("fullName"),
                        },
    ...
    def odds(self, league: str) -> dict[str, Any]:
        ...
        if api_key:
            ...
            try:
                ...
                return self._espn_embedded_odds(
                    league,
                    note=f"The Odds API unavailable ({exc}); using limited ESPN fallback. Replace or activate the Railway key.",
                )
        return self._espn_embedded_odds(
            league,
            note="Limited coverage; add a valid THE_ODDS_API_KEY for multi-book odds.",
        )
    ...
    def player_odds(self, league: str, max_events: int = 5) -> dict[str, Any]:
        ...
        _, _, sport_key = LEAGUES[league]
        try:
            events, usage = self._get_response(
            ...
            return self._result(
                ...
                [],
                note=f"Player props unavailable because provider authentication failed ({exc}). Replace or activate the Railway key.",
            )
        output = []
        markets = os.getenv(f"{league}_PLAYER_MARKETS", self.PLAYER_MARKETS.get(league, ""))
        for event in events[: max(1, min(max_events, 10))]:
            try:
                raw, call_usage = self._get_response(
                    f"https://api.the-odds-api.com/v4/sports/{sport_key}/events/{event['id']}/odds",
                    params={
                        "apiKey": api_key,
                        "regions": os.getenv("ODDS_REGIONS", "us"),
                        "markets": markets,
        ...
        return self._result(
            "The Odds API event odds",
            output,
            note=f"Player markets requested for up to {max_events} events.",
            usage=usage,
        )
    ...
    def lineup(self, league: str, event_id: str) -> dict[str, Any]:
        ...
        if league == "MLB":
            ...
            return self._result(
                ...
                note="Official batting orders generally appear near game time.",
            )
        sport, espn_league, _ = LEAGUES[league]
        raw = self._get(
            f"https://site.api.espn.com/apis/site/v2/sports/{sport}/{espn_league}/summary",
            params={"event": event_id},
        )
        return self._result(
            "ESPN event summary JSON",
            {
            ...
            note="Confirmed starters may not be published until close to event time.",
        )
